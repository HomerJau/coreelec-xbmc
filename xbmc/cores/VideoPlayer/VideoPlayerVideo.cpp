...
{
  CLog::Log(LOGDEBUG, "CVideoPlayerVideo::OpenStream - open stream with codec: {} fps:{:d}/{:d} options:{:02x}",
            avcodec_get_name(hint.codec), hint.fpsrate, hint.fpsscale, hint.codecOptions);
  ...
  if (!codec)
  {
    CLog::Log(LOGINFO, "CVideoPlayerVideo::OpenStream: - Creating video codec with codec: {} fps:{:d}/{:d} options:{:02x}",
              avcodec_get_name(hint.codec), hint.fpsrate, hint.fpsscale, hint.codecOptions);
    hint.pClock = m_pClock;
    hint.codecOptions |= CODEC_ALLOW_FALLBACK;
